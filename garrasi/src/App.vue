<template>
  ...
  <div v-for="user in users" style="margin-top: 80px; margin-bottom: 80px;">
    <div class="container user row">
      <div class="col">
        ...
        <button id="my-button" style="display:inline-block"><i class="fa fa-water"></i></button>
      </div>
      <div class="col-10">
        <div>
          <p class="test">{{ user.name }} | {{ user.date.getDate() }}/{{ user.date.getMonth() }}/{{
          (user.date.getFullYear()) }}</p>
        </div>
        <p>{{ user.comment }}</p>
      </div>
    </div>
  ...
  <div class="inputzone">
    <form action>
      ...
      <input type="text" id="lname" name="lname" placeholder="Your comment"><br><br>
    </form>
  </div>
</template>
<script>
import Navbar from "./views/NavBarView.vue";
// import url from "../public/background.js"
      ...
      users: [
        {
          ...
          comment: localStorage.getItem('url'),
          fire: -5
        },
        {
          name: "Kleinefrustuck",
